...
KinMedia::CircularBuffer::CircularBuffer( size_t capacity )
	: beg_index_( 0 ), end_index_( 0 ), size_( 0 ), capacity_( capacity )
{
	data_ = new char[capacity];
}

KinMedia::CircularBuffer::~CircularBuffer( )
{
	delete[] data_;
}

size_t KinMedia::CircularBuffer::write( const char * data, size_t bytes )
{
	if( bytes == 0 ) return 0;
...
size_t KinMedia::CircularBuffer::read( char * data, size_t bytes )
{
	if( bytes == 0 ) return 0;
